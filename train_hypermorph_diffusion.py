...
def image_loss(y_true, y_pred):
    ...
    y_true = y_true[..., 0, np.newaxis]  # ytrue must be fixed image
...
def grad_loss(y_true, y_pred):
    ...
    return hyp * vxm_sa.losses.Grad('l2', loss_mult=int_downsize).loss(None, y_pred, img_mask=None)
...
args.model_dir = "results/diffusion-hypermorph/" + dataset + "/" + model_name
...
model = vxm.networks.HyperVxmDense(inshape=inshape, nb_unet_features=[enc_nf, dec_nf], int_steps=int_steps,
                                   int_resolution=int_downsize, src_feats=nfeats, trg_feats=nfeats, svf_resolution=1, )
