...
import datasets
import utils
from regularizers import elastic_regularizer_modersitzki_tf_sa
...
def image_loss(y_true, y_pred):
    weight_matrix = y_true[...,1, np.newaxis]
    y_true=y_true[...,0, np.newaxis]
    weight_matrix = (1 - weight_matrix )  # mu + lambda <=1, # [bs, c]
    if crop_in_loss:
...
def grad_loss(y_true, y_pred):
    lam_matrix = y_true[..., 0]
    mu_matrix = y_true[..., 1]
    elastic_loss = elastic_regularizer_modersitzki_tf_sa(y_pred, lam_matrix, mu_matrix, img_mask=None, reduce=True, average=True)
    return elastic_loss
...
args.model_dir = "results/elastic-hypermorph-sa-vec/" + dataset + "/" + model_name
...
if dataset == "LungCTCT-2d":  # lung CT inh/exh
    datapath = data_base_path + "/LungCT"
    train_dataset = datasets.Learn2RegLungCTDataset(datapath, mode="train4", normalize_mode=True, dim_mode='2d')
    val_dataset = datasets.Learn2RegLungCTDataset(datapath, mode="val4", normalize_mode=True, dim_mode='2d')
    use_keypoints = False
    crop_in_loss=True
elif dataset == "NLST23":
    ...
    train_dataset = datasets.ACDCDataset(datapath, mode="train4", normalize_mode=True, roi_only=True, dim_mode='2d-random')
...
base_generator = utils.scan_to_scan_generator_sa(train_dataset, batch_size=args.batch_size) # gives back seg map as y_true[1]
val_base_generator = utils.scan_to_scan_generator_sa(val_dataset, batch_size=args.batch_size) # gives back seg map as y_true[1]
train_generator = utils.hyp_generator_elastic_sa_vector(args.batch_size, base_generator, num_classes)
val_generator = utils.hyp_generator_elastic_sa_vector(args.batch_size, val_base_generator, num_classes)
...
model = vxm.networks.HyperVxmDense(
    inshape=inshape,
    nb_hyp_params=len(train_dataset.classes)*2,
...
if loss_name == 'ncc':
    ...
    raise ValueError('Image loss should be "ncc", but found "%s"' % loss_name)
