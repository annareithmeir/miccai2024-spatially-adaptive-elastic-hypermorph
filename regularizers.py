    ...
    if batch:
        ...
        if dim == 3:  # u=[bs, y,x,z,3]
            ...
            if img_mask is not None:
                assert img_mask.shape[:-1]==u.shape[:-1]
...
def elastic_regularizer_modersitzki_tf_sa(u: tf.Tensor, lam_tensor:tf.Tensor, mu_tensor:tf.Tensor, pixel_spacing:Optional[np.ndarray]=None, img_mask:Optional[np.ndarray]=None, average : Optional[bool]=True, reduce: Optional[bool]=True, batch: Optional[bool]=True) -> tf.Tensor:
    """
    :param u: displacement field in shape of (bs, x,y,z,3) or (bs, x,y, 2)
    :param mu: Elasticity constant 1
    :param lam: Elasticity constant 2
    ...
    if batch:
        ...
        if dim == 2:  # u=[bs, 2,y,x]
            ...
            loss = dx[..., 0] + dy[..., 1]  # div
            loss = tf.math.square(loss) * (lam_tensor / 2)

            derivs = [dx, dy]

            for i in range(dim):
                for j in range(dim):
                    part1 = tf.square(tf.math.add(derivs[j][:, :, :, i], (derivs[i][:, :, :, j]))) * (mu_tensor / 4)
                    part1 = tf.squeeze(part1)
                    loss = tf.math.add(loss, part1)
