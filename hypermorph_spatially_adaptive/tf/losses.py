...
class NCC:
    ...
    def loss(self, y_true, y_pred, reduce='mean', weight_matrix=None, img_mask=None):
        # compute cc
        cc = self.ncc(y_true, y_pred)

        if weight_matrix is not None:
            print("in ncc(): weight_matrix is not None")
            cc= tf.math.multiply(cc, weight_matrix)

        if img_mask is not None:
            print("in ncc(): img_mask is not None")
            cc= tf.math.multiply(cc, img_mask)
...
class NCCSpatiallyAdaptive:
    ...
    def loss(self, y_true, y_pred, weight_matrix=None, img_mask=None, reduce='mean'):
        print("in nccsa loss", y_true.shape, y_pred.shape, weight_matrix.shape)
        # compute cc
        cc = self.ncc(y_true, y_pred)
...
class Grad:
    ...
    def loss(self, y_true, y_pred, weight_matrix=None, img_mask=None):
        """
        returns Tensor of size [bs]
        """

        if img_mask is not None:
            print(y_pred.get_shape(), img_mask.get_shape())
            y_pred = y_pred*img_mask
        ...
        if weight_matrix is not None:
            print("in grad loss():  weight_matrix given")
            if ndim==2:
                print("ppp", dif[0].shape, weight_matrix.shape)
                dif[0] = dif[0] * weight_matrix[ :, :-1, :]
                dif[1] = dif[1] * weight_matrix[ :, :, :-1]
            else:
                print("ppp", dif[0].shape, weight_matrix.shape)
                dif[0] = dif[0] * weight_matrix[:, :-1, :, :]
                dif[1] = dif[1] * weight_matrix[:,:, :-1, :]
                dif[2] = dif[2] * weight_matrix[:,:, :, :-1]
